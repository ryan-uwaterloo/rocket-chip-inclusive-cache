...
class MSHR(params: InclusiveCacheParameters) extends Module
{
  ...
  when (io.directory.valid || (io.allocate.valid && io.allocate.bits.repeat)) {
    ...
    printf(cf"@ clk_cycle ${clk_cycle}: Req in MSHR; need dram?: ${!new_meta.hit || (new_meta.state === BRANCH && new_needT)}, need probe? ${(new_meta.hit && ((new_needT || new_meta.state === TRUNK) && ((new_meta.clients & ~new_skipProbe) =/= 0.U)) && new_request.prio(0))}, evicting? ${!new_meta.hit && new_meta.state =/= INVALID}, back-inv? ${!new_meta.hit && new_meta.state =/= INVALID && (new_meta.clients =/= 0.U)}, source: 0x${new_request.source}%x, set: 0x${new_request.set}%x, tag: 0x${new_request.tag}%x, opcode: 0x${new_request.param}%x\n")
